...
#Collect info for workload cluster
echo "Enter name for the workload Cluster : "
read NKPCLUSTER
...
NETWORKS=$(govc find / -type Network)
echo
echo "Select second network to set as second nic"
select SECOND_NETWORK in $NETWORKS; do 
    echo "you selected network : ${SECOND_NETWORK}"
    echo 
    break
done
IFS=$SAVEIFS
